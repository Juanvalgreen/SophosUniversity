...
public class UserService implements IUserService{
    ...
    @Override
    public UserVerifyDto verifyUser(User user) throws Exception{
        try{
           ...
           if (argon2.verify(passwordHashed,user.getUser_password())){
               ...
               String token = jwtUtil.create(String.valueOf(list.get(0).getUser_id()),list.get(0).getUser_mail());
                ...
                return new UserVerifyDto(user.getUser_mail(), token);
           }
            
           
           return new UserVerifyDto(user.getUser_mail(), null);
